...
public class ClusteringNSGAII<S extends Solution<?>> implements
    Algorithm<List<S>> {
     ...
     * Constructor
     */
    public ClusteringNSGAII(Problem<S> problem, int maxEvaluations,
        int populationSize, CrossoverOperator<S> crossoverOperator,
        SelectionOperator<List<S>, S> selectionOperator,
        SolutionListEvaluator<S> evaluator, int numberOfParents) {
    ...
    public void run() {
        ...
        while (evaluations < maxEvaluations) {
            ...
            for (int i = 0; i < populationSize; i += 2) {
                ...
                // TODO: check if we need to generate two child solutions or
    ...
    protected List<S> crowdingDistanceSelection(Ranking<S> ranking) {
        ...
        while (populationIsNotFull(population)) {
            if (subfrontFillsIntoThePopulation(ranking, rankingIndex,
                ...
                crowdingDistance.computeDensityEstimator(ranking
                    .getSubfront(rankingIndex));
    ...
    protected boolean subfrontFillsIntoThePopulation(Ranking<S> ranking,
                                                     int rank,
                                                     List<S> population) {
    ...
    protected void addLastRankedSolutionsToPopulation(Ranking<S> ranking,
                                                      int rank,
                                                      List<S> population) {
        ...
        Collections.sort(currentRankedFront,
            new CrowdingDistanceComparator<S>());
